...
export function App() {
  ...
  return <div className="appShell"><Sidebar /><main className="main"><TopNav /><LegacyImportBanner />{page !== 'overview' && <BossHero />}{page === 'overview' && <OverviewPage />}{page === 'tactics' && <TacticsPage />}{page === 'raid' && <RaidPage />}{page === 'planner' && <PlannerWorkspace editable />}{page === 'timeline' && <TimelinePage />}{page === 'roster' && <RosterPage />}{page === 'notes' && <NotesPage />}<footer className="siteFooter">RaidRU 3 Architecture Core · WCL и RaidPlan будут подключены через отдельные адаптеры, а не через патчи UI.</footer></main></div>;
}
