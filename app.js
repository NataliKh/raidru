...
const ARENA_MASK_VERSION='0.7.11-hard-masks-v2';
...
function exportPlan(){const blob=new Blob([JSON.stringify({version:'0.7.11',boss:current,diff,role,data:bossState(current)},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`raidru-${current}-v0711.json`;a.click();URL.revokeObjectURL(a.href);toast('Стратегия экспортирована')}
...
function backupAll(){const blob=new Blob([JSON.stringify({version:'0.7.11',savedAt:new Date().toISOString(),state},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='raidru-backup-v05.json';a.click();URL.revokeObjectURL(a.href);toast('Резервная копия скачана')}
