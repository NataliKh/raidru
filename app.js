...
function normalizeScene(sc,id,i){
  ...
  if(!sc.map) sc.map={zoom:100,x:0,y:0,dark:18};
...
function dashboard(){return `<section class="page"><div class="stats"><div><b>${raid.filter(x=>bossState(x.id).progress>=100).length}/8</b><span>убито</span></div><div><b>${raid.filter(x=>bossState(x.id).favorite).length}</b><span>в избранном</span></div><div><b>${rosterState().length}</b><span>в составе</span></div><div><b>${raid.reduce((a,x)=>a+bossState(x.id).scenes.length,0)}</b><span>сцен</span></div></div><div class="bossCards">${raid.map(x=>{const st=bossState(x.id);return `<article onclick="openBoss('${x.id}')"><div class="bossThumb"><img src="${arenaMaps[x.id]}" alt="" loading="lazy"><span>${x.order}</span></div><div><small>${x.en}</small><h3>${x.name}</h3><p>${x.summary}</p><div class="progressLine"><i style="width:${st.progress}%"></i></div><footer><span>${st.progress}% освоено</span><b>${st.scenes.length} сцен · ${st.timelineV3.length} событий</b></footer></div></article>`}).join('')}</div><div class="dashboardActions"><button onclick="backupAll()">Скачать резервную копию RaidRU</button><button onclick="resetAllConfirm()">Сбросить локальные данные</button><span>0.5 хранит планы локально; ссылки «Поделиться» включают сцены, эффекты, маршруты и таймлайн.</span></div></section>`}
function continueRaid(){const x=raid.find(x=>bossState(x.id).progress<100)||raid[0];openBoss(x.id)}
...
function arenaMapHtml(id,sc){
  ...
  const m=sc?.map||{zoom:100,x:0,y:0,dark:18};
