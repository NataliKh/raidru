...
## RaidPlan visual fidelity fix

- Вкладка RaidPlan теперь использует **фон арены самого RaidPlan**, а не WCL/mapID-подложку RaidRU. Основная вкладка RaidRU не менялась.
- `itext` распознаётся раньше ролей: слова «танк», «РДД», «хил» внутри инструкции больше не превращают текст в игрока.
- Роли определяются по настоящим `game/wow/role/*.svg`; `markerStyle: square` больше не превращает их в синюю квадратную raid-метку.
- Raid markers используют исходные изображения RaidPlan и исходный масштаб.
- `mob` использует `displayId`-портрет RaidPlan, исходный размер, красное кольцо и указатель направления.
- Текст сохраняет переносы строк, ширину/высоту, размер шрифта и базовое форматирование (bold/underline/italic).
- `circle` сохраняет исходный fill; `line`/стрелки сохраняют цвет, толщину и направление. Автоматические подписи «Стрелка RaidPlan»/«Линия RaidPlan» для таких объектов убраны.
- `step_notes_raw` импортируется как настоящая заметка сцены; текстовые блоки карты больше не дублируются в textarea.
- Внутри вкладки RaidPlan strip сцен стал компактным: номера `1…N`, название доступно в tooltip и поле имени сцены.
- Координатная система 1200×675 → проценты оставлена без повторной калибровки: сравнение первого кадра показало, что позиции объектов уже совпадают.
...
- Название сцены берётся из верхнего `itext`; заметка шага берётся отдельно из `step_notes_raw`.
